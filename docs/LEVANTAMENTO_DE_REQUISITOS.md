...
| `RF-INS-004` | Ao surgir vaga, o sistema DEVE oferecê-la ao primeiro aluno elegível da fila. | Somente uma oferta ativa ocupa a vaga. A oferta gera uma notificação interna obrigatória e inicia um prazo de 48 horas corridas, inclusive fins de semana e feriados. O aluno confirma ou recusa somente na área autenticada `Minhas ofertas`; mensagens apenas direcionam para essa tela. Apagar a notificação não cancela nem oculta a oferta ativa. Confirmação efetiva a inscrição; recusa ou expiração passa a vaga ao próximo sem duplicidade. WhatsApp, quando disponível em versão posterior, será aviso adicional. | `RN010`, `RN011`, `SE034` | Crítica | Canal interno na primeira versão; WhatsApp depende de fornecedor futuro |
...
| `RF-INS-007` | Turma configurada com processo seletivo DEVE encaminhar solicitações a um painel Kanban. | Critérios são textos versionados por modalidade e faixa etária, identificados como obrigatórios ou opcionais. O professor ou administrador responsável marca `ATENDEU` ou `NÃO_ATENDEU` e registra a decisão final humana; nenhum resultado aprova ou reprova automaticamente. A candidatura usa `INSCRITO`, `EM_ANÁLISE`, `APROVADO`, `REPROVADO` ou `CANCELADO`. Não há anexos nem recurso do aluno. Correção de erro é exclusiva do administrador total. | `SE025`, `RN018` | Alta | Permissão do professor e tratamento da capacidade em refinamento no Diagrama de Atividades |
...
| `RN-011` | Oferta deve ser confirmada em até 48 horas corridas; expirada, segue ao próximo. | O prazo começa quando a oferta e sua notificação interna são registradas e corre continuamente, inclusive em fins de semana e feriados. Relógio, confirmação, recusa e expiração ficam registrados. Falha do WhatsApp não reinicia nem suspende o prazo. | Definido em 14/08/2026 |
...
| `RN-018` | Turma pode exigir processo seletivo; solicitação depende de decisão humana via Kanban. | O fluxo usa `INSCRITO → EM_ANÁLISE → APROVADO/REPROVADO/CANCELADO`. Critérios obrigatórios ou opcionais orientam o professor ou administrador responsável, mas não produzem decisão automática. Na primeira versão não haverá anexos nem recurso do aluno. Erro pode ser corrigido somente por administrador total, com motivo e auditoria. Antes da matrícula, a correção é direta; depois, aplica-se compensação e capacidade. | Definido parcialmente; autorização do professor e capacidade serão concluídas em `ATIVIDADES.md` |
...
| `DEC-011` | A oferta de vaga terá prazo de 48 horas corridas a partir do registro da oferta e da notificação interna. | Decisão da equipe em 14/08/2026 | Aceita |
...
| `Q-003` | A oferta de vaga dura 48 horas corridas, iniciadas com o registro da oferta e da notificação interna. Falha do WhatsApp não altera o prazo. | WhatsApp ficou fora da primeira versão em `Q-009`; o canal interno é suficiente para o fluxo inicial. |
...
| --- | --- | --- |
| `DEC-079` | No processo seletivo, cada critério informa se é obrigatório ou opcional, mas nenhum resultado decide automaticamente. Professor ou administrador responsável registra a decisão humana final. | Aceita em 14/08/2026; permissão do professor ainda em refinamento |
...
| `0.2.0` | 14/08/2026 | Heitor Leite | Incorporação das decisões da equipe sobre regras, segurança, privacidade, notificações, capacidade, fluxos críticos, primeira versão e infraestrutura Prefeitura/Embrass; ajustes transversais confirmados durante casos de uso e modelagem de classes | Pronto para revisão |
